...
CTrack SalesmanTrackBranchAndBound3(CGraph& graph, CVisits& visits) {
	...
	for (int column = 1; column <= n; column++) {
		...
		double maxCVj = 0;
		for (int raw = 0; raw <= n; raw++) {
			if (raw != column /*&& raw != n*/ && column != 0 && (column != n || raw != 0)) {
				if (dist_table[raw][column] > maxCVj) {
					maxCVj = dist_table[raw][column];
				}
				if (dist_table[raw][column] < minCVj) {
	...
	cout << endl;
	p_q.push(p);
	double cota_sup_global = p->cota_sup + 10e-5;
	priority_queue<double, vector<double>, greater<double>> list_cota_sup;
	Path optimum = Path(&graph);
	while (!p_q.empty()) {
		Path* top = p_q.top();
		p_q.pop();
		...
		if (top->m_Unvisited.size() == 1) {
			...
			if (dist_table[last][j] > maxCVj) {
				maxCVj = dist_table[last][j];
			}
			if (dist_table[last][j] < minCVj) {
				minCVj = dist_table[last][j];
			}
			Path* p = new Path(*top);
			p->AddVertex(n);
			p->cota_inf += dist_table[last][n] - minCVj;
			...
			for (int j : top->m_Unvisited) {
				if (j != n) {
					...
					p->cota_inf += dist_table[last][j] - minCVj;
					p->cota_sup += dist_table[last][j] - maxCVj;
					cout << "coat sup global : " << cota_sup_global << endl;
					cout << "cota inf : " << dist_table[last][j] << " - " << minCVj << " = " << p->cota_inf << endl;
					cout << "cota sup : " << dist_table[last][j]<<" - "<<maxCVj<<" = "<<p->cota_sup << endl << endl;
					if (p->cota_inf <= cota_sup_global) {
						p_q.push(p);
						list_cota_sup.push(p->cota_sup);
					}
				}
			}
			cota_sup_global = list_cota_sup.top() + 10e-5;
			while (!list_cota_sup.empty()) {
				list_cota_sup.pop();
			}
		}
		if (p_q.top()->m_Vertices.size() == n + 1) {
