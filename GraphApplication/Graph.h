...
class Path {
public:
	vector<int> m_Vertices; // vector d'index plutôt que de sommets
	vector<int> m_Unvisited;
	CGraph* m_pGraph;
	double m_length;
	double cota_inf=0;
	double cota_sup=0;
	...
	void AddVertex(int v) {
		...
		if (v == m_Unvisited.back()) {
			m_Unvisited.pop_back();
		}
		else {
			for (auto it = m_Unvisited.begin(); it != m_Unvisited.end();) {
				if (*it == v) {
					std::swap(*it, m_Unvisited.back());
					m_Unvisited.pop_back();
				}
				else {
					++it;
...
struct Comp {
	bool operator()(Path* p1, Path* p2) {
		return p1->cota_inf > p2->cota_inf;
	}
};
